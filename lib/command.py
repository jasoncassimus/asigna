...
import data.world_data
from data.world_data import add_continent, add_zone
...
class Commands(object):
    ...
    def execute_command(self, player, command, param):
        if player.name is None:
            pass
        elif self.commands.get(command):
            self.commands[command](player, param)
        else:
            player.message("Unknown command '{}'".format(command))

    def new(self, player: Player, params):
        target = params.split(' ')[0]
        if target == "continent":
            new_continent = Continent()
            new_continent.createdBy = player.name
            target = params.split(' ')[1]
            new_continent.name = target
            data.world_data.add_continent(new_continent)
            data.world_data.print_table("world")
        elif target == "zone":
            new_zone = Zone()
        elif target == "room":
            new_room = Room()
            new_room.name = "A New Room"
